...
;      - 00F0h - 00FFh = Used for the menu selector
...
os_call_vectors:
	...
	jmp os_illegal_call			; 8093h ; FREE!!!!!!!!!!!!!!!!!!!
	...
	jmp os_illegal_call			; 80DEh ; FREE!!!!!!!!!!!!!!!!!!!
...
;	call os_modify_int_handler
	...
	mov cx, 04h					; INTO instruction error handler
	mov di, 1000h
	mov si, os_compat_int04
	call os_modify_int_handler

	mov cx, 05h					; BOUND instruction error handler
	mov di, 1000h
	mov si, os_compat_int05
	call os_modify_int_handler

	mov cx, 06h					; Bad instruction error handler
	mov di, 1000h
	mov si, os_compat_int06
	call os_modify_int_handler

	mov cx, 07h					; Processor extension error handler
	mov di, 1000h
	mov si, os_compat_int07
	call os_modify_int_handler

	mov cx, 1Ch					; RTC handler
	mov di, 1000h
...
systemfilemissing:
	...
	appindex1				dw edit_name, spreadst_name, viewer_name, calc_name, clock_name
	...
	gameoptions				db 'InkSpill,Cosmic Flight,Space Inventors', 0
	...
	progoptions				db 'File Manager,Text Editor,Spreadsheet Editor,Image Viewer,Calculator,Clock,Terminal,Settings,Pixel art editor,Music player,Hardware checker,About MichalOS,Debug tools', 0
